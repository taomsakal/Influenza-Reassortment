...
anim = ggplot(data=total, mapping=aes(x=Key, y=Value, fill=Species, frame = Step))+
        geom_bar(stat = "identity", position = "stack", width=1.2) +
        geom_text(aes(y = -1, label = Key, vjust = 0.25, hjust = 1, angle = 90, family = "Arial Narrow"), size = 3) +
        geom_text(aes(y = as.integer(sub(".", "", label)), label = label, hjust = 0, vjust = 0.25, angle = 90, family = "Arial Narrow"), size = 3) +
        ...
        ease_aes("sine-in-out") + coord_fixed(0.3)

animate(anim, nframes = 10,fps = 5,  width = 1500, height = 900, 
